  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" persistent max-width="800px">
      <template v-slot:activator="{ on }">
        <v-btn outlined color="teal lighten-3" dark v-on="on">Refakatçi ekle</v-btn>
      </template>
      <v-card>
        <v-card-title>
          <span class="headline">Refakatçi ekle</span>
        </v-card-title>
            ...
            <label for="exampleInputPassword1" class="form-label">Şifre</label>
              <input 
                ...
                <div v-if="submitted && $v.password.$error" class="invalid-feedback">
                  ...
                  <div v-if="!$v.password.minLength" id="emailHelp" class="form-text text-danger">Şifre en az 6 haneli olmalıdır</div>
                  <div v-if="!$v.password.maxLength" id="emailHelp" class="form-text text-danger">Şifre en az 6 haneli olmalıdır</div>
                </div>
          </div>
          
          <div class="d-flex justify-content-center">
            <button type="submit" class="btn btn-success w-100 mt-2">Refakatçi ekle</button>
    ...
    methods: {
      ...
      axios.post(url,form,{
        ...
        if(res.data.code=='200') {
          Vue.$toast.success('hasta yakını başarıyla eklendi')
