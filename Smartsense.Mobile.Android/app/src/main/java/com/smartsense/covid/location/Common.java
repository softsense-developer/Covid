...
public class Common {
    ...
    public static String getLocationText(Location mLocation) {
          return mLocation==null ? "Bilinmeyen Konum" : new StringBuilder()
    ...
    public static CharSequence getLocationTitle(MyBackgroundService myBackgroundService) {
        return String.format("Konum Güncellendi: %1$s", DateFormat.getDateInstance().format(new Date()));
    }

    public static void setRequestingLocationUpdates(Context context, boolean value) {
        PreferenceManager.getDefaultSharedPreferences(context).edit().putBoolean(KEY_REQUESTING_LOCATION_UPDATES,value)
                .apply();
    }
